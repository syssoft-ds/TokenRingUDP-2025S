        ...
        try(BufferedReader br = new BufferedReader(new InputStreamReader(System.in))) { // closes automatically
            ...
            while (!(input = br.readLine()).equalsIgnoreCase("quit")) {
                ...
                if (parts[0].equalsIgnoreCase("register") && parts.length == 3 && isPort(parts[2])) {
                    ...
                    if (clients.isEmpty()) {
                        System.err.println("No registered clients to send to.");
                    } else {
                        for (Map.Entry<String, ClientInfo> entry : clients.entrySet()) {
                            String clientName = entry.getKey();
                            ClientInfo info = entry.getValue();
                            sendLines(info.ip, info.port, name + " (broadcast): " + message);
                        }
                        System.out.println("Broadcast message sent to all registered clients.");
                    }
                }
        ...
        try(DatagramSocket s = new DatagramSocket(port)) { // closes automatically
            ...
            do {
                ...
                if (predefinedResponses.containsKey(line)) {
                    sendLines(p.getAddress().getHostAddress(), p.getPort(), predefinedResponses.get(line));
                    continue;
                }
